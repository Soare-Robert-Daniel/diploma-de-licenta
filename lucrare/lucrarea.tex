...
Pentru evaluarea vectorului vom folosii func@tia \texttt{predict} definit@a de \texttt{sequential}. Este important s@a re@tinem faptul c@a \texttt{predict} trebuie s@a primeasc@a un vector de date de intrare, iar rezultatul s@au este un vector de date de i@sire. A@sadar, vectorul nostru de 3 elemente trebuie pus @intr-un alt vector, care @in final art@a precum o matrice care va fi transformat @intr-un tensor 2-dimensional prin folosirea func@tiei \texttt{tensor2d}. Biblioteca Tensorflow lucrea @in principal cu tensori a@sa c@a este foarte important s@a transmitem date @in formatul corect. Dac@a am fi avut o matrice ca date de intrare, atunci ar fi trebuit s@a cre@am un tensor 3-dimensional.
Ini@tiniera valorilor ponderilor sinaptice este aleatoare, @si pentru vizualizarea lor punem ini@tia @in datele de intrare o valoarea cu 1 @si restul cu 0.
...
Se poate observa c@a pe linia 8 se va afi@sa suma valoriloe de pe liniile 2, 4, 6. Vom adauga o func@tie de activare @si anume \textit{relu} (Rectified Linear Unit), descris @in capitolul 2. Acesta func@tie de activare transforma valorile negative @in valoarea 0, astfel av@and un rol de filtrare a valorilor negative. Exemplu:
